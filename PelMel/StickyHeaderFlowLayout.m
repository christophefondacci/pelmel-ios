...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
            ...
            if(contentOffset.x == 0 && indexPath.section==0) {
                frame.origin.x = 0;
            }
            // Width will stretch to the next section up to screen width bounds max
            frame.size.width = MAX(100,MIN(nextHeaderOrigin.x - frame.origin.x,contentOffset.x+self.collectionView.frame.size.width - frame.origin.x));
            // Height is the one defined as reference
            frame.size.height=self.headerReferenceSize.height;
        }
